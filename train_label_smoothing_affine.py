...
def add_noise_to_labels(labels, noise_min= 0.5, noise_max= 10):
    noise = torch.empty_like(labels).normal_(mean=0, std=1)
    noise = noise_min + (noise_max - noise_min) * (labels / 100) * noise # Bruit personnalisé pour chaque label selon une fonction affine
    noisy_labels = torch.clamp(labels + noise, min=0, max=100) # Clamp pour rester dans les âges valides
    noisy_labels = noisy_labels.round().int()
    return noisy_labels.round()
...
def get_args():
    ...
    parser.add_argument("--noise_min", type=float, default=0.5, help="Noise used for the minimum age value")
    parser.add_argument("--noise_max", type=float, default=10, help="Noise used for the maximum age value")
...
def train(train_loader, model, criterion, optimizer, epoch, device, noise_min=0.5, noise_max=10.0):
    model.train()
    ...
    with tqdm(train_loader) as _tqdm:
        for x, y in _tqdm:
            x = x.to(device)
            y = y.to(device)
            y_noisy = add_noise_to_labels(y.float(), noise_min, noise_max)  # Ajoute du bruit uniquement aux labels d'entraînement
            y_noisy = y_noisy.type(torch.LongTensor)
            y_noisy = y_noisy.to(device)
            # compute output
            outputs = model(x)

            # calc loss
            loss = criterion(outputs, y_noisy)
            cur_loss = loss.item()
...
def main():
    ...
    for epoch in range(start_epoch, 50):
        # train
        train_loss, train_acc = train(train_loader, model, criterion, optimizer, epoch, device, args.noise_min, args.noise_max)
