...
def add_noise_to_labels(labels, noise_std=1.5):
    noise = torch.randn_like(labels) * noise_std  # Ajoute un bruit gaussien
...
def get_args():
    ...
    parser.add_argument("--noise_std", type=float, default=2.0, help="Standard deviation of Gaussian noise for label smoothing")
...
def train(train_loader, model, criterion, optimizer, epoch, device, noise_std=1.5):
    model.train()
    ...
    with tqdm(train_loader) as _tqdm:
        for x, y in _tqdm:
            x = x.to(device)
            y = y.to(device)
            y_noisy = add_noise_to_labels(y.float(), noise_std)  # Ajoute du bruit uniquement aux labels d'entraînement
...
def main():
    ...
    for epoch in range(start_epoch, 50):
        # train
        train_loss, train_acc = train(train_loader, model, criterion, optimizer, epoch, device, args.noise_std)
